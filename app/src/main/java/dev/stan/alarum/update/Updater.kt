...
import dev.stan.alarum.BuildConfig
import java.io.File
 ...
 *
 * Driven entirely by the Settings buttons — nothing here runs on its own. An
 * alarm clock that silently replaced itself in the small hours would be a
 * spectacular way to miss a morning.
 *
 * A process-wide singleton so a download survives leaving the Settings screen.
 */
class Updater private constructor(private val app: Context) {
    ...
    /** True once the user has allowed Alarum to install apps. */
    ...
    private suspend fun download(release: Release): File = withContext(Dispatchers.IO) {
        ...
        val target = File(dir, "alarum-${release.tag}.apk")
    ...
    private suspend fun install(apk: File) = withContext(Dispatchers.IO) {
        ...
        installer.openSession(sessionId).use { session ->
            session.openWrite("alarum", 0, apk.length()).use { out ->
